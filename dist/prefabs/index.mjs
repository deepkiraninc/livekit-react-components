...
function useWhiteboard() {
  ...
  const room = useRoomContext();
  const [isWhiteboardShared, setIsWhiteboardShared] = React37.useState(false);
  const [isWhiteboardHost, setIsWhiteboardHost] = React37.useState(true);
  React37.useEffect(() => {
...
import { isEqualTrackRef, isTrackReference as isTrackReference3, isWeb, log as log10, setupParticipantName as setupParticipantName3 } from "@livekit/components-core";
...
import { isTrackReference as isTrackReference2, isTrackReferencePinned } from "@livekit/components-core";
...
function useIsEncrypted(participant) {
  ...
  const observer = React68.useMemo(() => encryptionStatusObservable(room, p), [room, p]);
...
function ParticipantTile(_a) {
  ...
  ) : isTrackReference2(trackReference) && /* @__PURE__ */ React70.createElement(
    AudioTrack,
    {
      trackRef: trackReference,
      onSubscriptionStatusChanged: handleSubscribe
...
function formatChatMessageLinks(message) {
  return tokenize(message, createDefaultGrammar()).map((tok, i) => {
    if (typeof tok === `string`) {
      ...
      return /* @__PURE__ */ React84.createElement("span", { key: i, dangerouslySetInnerHTML: { __html: html } });
    } else {
...
function WhiteboardIndicater({
  ...
  return /* @__PURE__ */ React90.createElement("button", { disabled: disableWhiteboard, className: "tl-blur lk-button", onClick: toggleWhiteboard }, "Whiteboard ", state == null ? void 0 : state.show_whiteboard);
}
...
function VideoConference(_a) {
  ...
  const screenShareTracks = tracks.filter(isTrackReference3).filter((track) => track.publication.source === Track12.Source.ScreenShare);
